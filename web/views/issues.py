...
class CheckFilter(object):
    ...
    def __iter__(self):
        for item in self.data_list:
            ...
            param_url = query_dict.urlencode()  # 取url的参数值
            if param_url:
...
def issues_record(request, project_id, issues_id):
    ...
    if request.method == 'GET':
        ...
        for row in reply_list:
            data = {
                ...
                'datetime': row.create_datetime.strftime("%Y-%s-%d %H-%M"),
    ...
    if form.is_valid():
        ...
        info = {
            ...
            'datetime': instance.create_datetime.strftime("%Y-%s-%d %H-%M"),
...
def issues_change(request, project_id, issues_id):
    ...
    field_object = models.Issues._meta.get_field(name)  # 拿到model表中的name对应的字段实列
