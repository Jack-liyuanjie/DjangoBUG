...
from web.forms.file import FolderModelForm, FileModeForm
...
# http://127.0.0.1:8000/manage/2/file/?fid=1
def file_delete(request, project_id):
    ...
    if delete_object.file_type == 1:
        # 删除文件 (数据库文件删除， cos文件删除， 项目使用的空间容量还回去)

        # 删除文件，将容量返还给当前项目的已使用空间
    ...
    # 删除文件夹 (找到文件夹下所有的文件--->数据库文件删除，cos文件删除，项目空间返回)
    # 找当前目录下的文件和文件夹，文件放一个列表，文件夹放一个列表，然后循环遍历文件夹
    total_size = 0
    key_list = []
    ...
    # cos批量删除文件
    if key_list:
        delete_file_list(request.project.bucket, request.tracer.project.region, key_list)
...
def file_post(request, project_id):
    ...
    # 通过ModelForm.save储存到数据库中的返回的数据返回的instance对象，无法通过get__xx_display获取choice的中文
    form = FileModeForm(request, data=request.POST)
    if form.is_valid():
