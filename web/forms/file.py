...
class FolderModelForm(BootStrapForm, forms.ModelForm):
    ...
    def clean_name(self):
        ...
        # 数据库判断，当前目录下，此文件夹是否存在
        qureyset = models.FileRepository.objects.filter(file_type=2, name=name, project=self.request.tracer.project)
        if self.parent_object:
            exists = qureyset.filter(parent=self.parent_object).exists()
        else:
            exists = qureyset.filter(parent__isnull=True).exists()
        if exists:
